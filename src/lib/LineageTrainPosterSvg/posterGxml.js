 ...
 * @param {*} coverEls Array of gxml objects defining the cover region elements
...
export function posterGxml(layout, contentEls, preambleEls=[], borderEls=[],
        headerEls=[], footerEls=[], coverEls=[], guideEls=[]) {
    const {sheet, border, header, footer, left, right, content, cover} = layout
    ...
    const borderSvg = {el: 'svg', id: 'border-svg',
        x: border.x, y: border.y, width: border.width, height: border.height,
        els: borderEls
    }

    const headerSvg = {el: 'svg', id: 'header-svg',
        x: header.x, y: header.y, width: header.width, height: header.height, els: headerEls
    }

    const leftSvg = {el: 'svg', id: 'left-svg',
        x: left.x, y: left.y, width: left.width, height: left.height, els: []
    }
    ...
    const footerSvg = {el: 'svg', id: 'footer-svg',
        x: footer.x, y: footer.y, width: footer.width, height: footer.height,
        els: footerEls
    }

    const rightSvg = {el: 'svg', id: 'right-svg',
        x: right.x, y: right.y, width: right.width, height: right.height, els: []
    }

    const coverSvg = {el: 'svg', id: 'cover-svg',
        x: cover.x, y: cover.y, width: cover.width, height: cover.height,
        els: coverEls
    }

    const all = [preambleEls, sheetSvg, borderSvg, headerSvg, footerSvg, leftSvg, rightSvg,
        contentSvg, coverSvg, guidesSvg]

    const els = layout.portrait ? all
        : [{el: 'g', transform: `translate(${sheet.height},0) rotate(90)`, els: all}]
    
    const topSvg = {el: 'svg',  xmlns: "http://www.w3.org/2000/svg",
        width: sheet.width, height: sheet.height, els: els}
