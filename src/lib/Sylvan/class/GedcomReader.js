 ...
 */
import { GedcomRecords } from '../class/GedcomRecords.js'
...
export class GedcomReader {
    ...
    _parseRecord(line) {
        ...
        if (fields.length < 2) {
            this._addMsg(`Line ${this.lineNo()} has less than 2 fields: [${line}]`)
        }
        ...
        if (this._isKey(fields[1])) {
            ...
            if (fields.length < 3) {
                throw new Error(`Line ${this.lineNo()} has a key field '${fields[1]}' but no type field`)
            }
